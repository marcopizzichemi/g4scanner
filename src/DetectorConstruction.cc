...
: G4VUserDetectorConstruction()
{
  ...
  //in witdh we want it twice the biggest width, so we need to check if the matrix or the mppcs are the biggest ones
...
G4VPhysicalVolume* g4matrixDetectorConstruction::Construct()
{
  ...
  // //world dimensions
  // //lenght is easy, it will have to accomodate for crystal length plus source distance plus source radius (if any) plus some space (typically x2)
  // fExpHall_z = (fAirThinLayerBox_z + distance + sphereRadius)*4.0;
  // //in width we want it twice the biggest width, so we need to check is the matrix or the mppcs or the source are the biggest ones
  // std::vector<double> sizeX;
  // sizeX.push_back(fAirThinLayerBox_x * nCrystalsX);
  // sizeX.push_back(fMPPCArray_x);
  // sizeX.push_back(fabs(sourcex) + sphereRadius);
  // std::sort(sizeX.begin(), sizeX.end());
  //
  // std::vector<double> sizeY;
  // sizeY.push_back(fAirThinLayerBox_y * nCrystalsY);
  // sizeY.push_back(fMPPCArray_y);
  // sizeY.push_back(fabs(sourcey) + sphereRadius);
  // std::sort(sizeY.begin(), sizeY.end());
  //
  // std::cout<< "---------------------------------->" << sizeX[0] << " "  <<sizeX[1] << " " << sizeX[2] << std::endl;
  //
  // //teflon box
  // fExpHall_x = 4.0 * sizeX[2];
  // fExpHall_y = 4.0 * sizeY[2];
  //
  // fTeflonExt_x = sizeX[2] + 2.0*esrTeflonGap + 2.0*teflonThickness;
  // fTeflonExt_y = sizeY[2] + 2.0*esrTeflonGap + 2.0*teflonThickness;
  // fTeflonInt_x = sizeX[2] + 2.0*esrTeflonGap ;
  // fTeflonInt_y = sizeY[2] + 2.0*esrTeflonGap ;

  // for z, we calculate later from the position of the first and last volumes in z (mppc and fakeAir)
  ...
  // fTeflonExt_z = ((pMPPCArray_z + fMPPCArray_z/2.0) - (pFakeAirBack_z + fFakeAirBack_z/2.0)) + esrTeflonGap + teflonThickness;
  // fTeflonInt_z = ((pMPPCArray_z + fMPPCArray_z/2.0) - (pFakeAirBack_z + fFakeAirBack_z/2.0)) + esrTeflonGap;
  //
  // std::cout << "fTeflonInt_x = " << fTeflonInt_x << std::endl
  //           << "fTeflonInt_y = " << fTeflonInt_y << std::endl
  //           << "fTeflonInt_z = " << fTeflonInt_z << std::endl
  //           << std::endl;
  // std::cout << "fTeflonExt_x = " << fTeflonExt_x << std::endl
  //           << "fTeflonExt_y = " << fTeflonExt_y << std::endl
  //           << "fTeflonExt_z = " << fTeflonExt_z << std::endl
  //           << std::endl;
...
//   bubble_dim_z = fCrystal_z/2.0 + frontInterface + fEpoxy_z + fMPPCArray_z + fCrystal_z/2.0 + fGreaseBackCryToGlass_z + fGlassBack_z + fAirBack_z + fFakeAirBack_z;
  ...
  G4double RefractiveIndex1[nEntries1] = { greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex, greaseIndex };
  ...
  {
    1.2407002545,    1.2532325803,    1.2660206679,    1.2790724273,    1.2923960984,    1.3060002679,    1.3198938878,    1.3340862952,    1.3485872332,    1.3634068731,    1.3785558383,    1.3940452298,    1.4098866528,    1.4260922466,    1.4426747145,    1.4596473582,    1.4770241125,    1.4948195837,    1.5130490909,    1.5317287093,    1.5508753181,    1.5705066513,    1.5906413519,    1.6112990318,    1.6325003349,    1.654267006	,    1.6766219655,    1.6995893897,    1.7231947979,    1.7474651472,    1.772428935	,    1.7981163109,    1.8245591978,    1.8517914246,    1.8798488705,    1.9087696223,    1.9385941477,    1.9693654833,    2.0011294428,    2.0339348435,    2.0678337575,    2.1028817873,    2.1391383698,    2.1766671132,    2.2155361688,    2.2558186446,    2.2975930639,    2.3409438764,    2.3859620279,    2.4327455971,    2.481400509	,    2.5320413357,    2.5847921969,    2.6397877755,    2.6971744663,    2.7571116767,    2.8197733057,    2.8853494291,    2.954048225	,    3.0260981817,    3.1017506363,    3.1812827039,    3.2650006698,    3.3532439311,    3.4463895958,    3.54485787	,    3.6491183956,    3.7596977409,    3.8771882953,    4.0022588855,    4.135667515	,    4.2782767397,    4.4310723375,    4.5951861278,    4.7719240558,    4.962801018
  };
  G4double siliconRindexValue[76] =
  {
    3.57,     3.574,    3.578,    3.582,    3.587,    3.592,    3.597,    3.602,    3.608,    3.614,    3.62 ,    3.626,    3.632,    3.638,    3.644,    3.65 ,    3.656,    3.662,    3.668,    3.674,    3.681,    3.688,    3.696,    3.705,    3.714,    3.723,    3.732,    3.741,    3.751,    3.762,    3.774,    3.787,    3.8  ,    3.815,    3.83 ,    3.844,    3.861,    3.879,    3.895,    3.916,    3.939,    3.962,    3.986,    4.015,    4.044,    4.077,    4.11 ,    4.15 ,    4.192,    4.239,    4.293,    4.348,    4.416,    4.491,    4.577,    4.676,    4.793,    4.925,    5.091,    5.305,    5.587,    5.976,    6.548,    6.863,    6.014,    5.483,    5.293,    5.179,    5.102,    5.074,    5.055,    4.426,    3.052,    2.129,    1.8  ,    1.694
  };
  G4double siliconAbsLenghtValue[76] =
  {
    1.56E-02,    1.26E-02,    1.04E-02,    8.77E-03,    7.46E-03,    6.37E-03,    5.46E-03,    4.76E-03,    4.17E-03,    3.68E-03,    3.27E-03,    2.92E-03,    2.61E-03,    2.31E-03,    2.08E-03,    1.87E-03,    1.69E-03,    1.55E-03,    1.41E-03,    1.29E-03,    1.18E-03,    1.08E-03,    9.90E-04,    9.09E-04,    8.40E-04,    7.69E-04,    7.04E-04,    6.49E-04,    6.02E-04,    5.65E-04,    5.26E-04,    4.88E-04,    4.52E-04,    4.20E-04,    3.88E-04,    3.56E-04,    3.29E-04,    3.06E-04,    2.84E-04,    2.62E-04,    2.42E-04,    2.23E-04,    2.05E-04,    1.88E-04,    1.73E-04,    1.56E-04,    1.42E-04,    1.27E-04,    1.14E-04,    1.03E-04,    9.01E-05,    7.87E-05,    6.76E-05,    5.81E-05,    4.76E-05,    3.92E-05,    3.22E-05,    2.55E-05,    2.00E-05,    1.48E-05,    1.05E-05,    6.67E-06,    3.41E-06,    1.43E-06,    9.80E-07,    9.62E-07,    9.17E-07,    8.55E-07,    7.81E-07,    6.94E-07,    5.78E-07,    4.46E-07,    4.24E-07,    4.59E-07,    5.08E-07,    5.43E-07
  };
  ...
  G4VPhysicalVolume* expHall_phys = new G4PVPlacement(0,G4ThreeVector(),expHall_log,"World",0,false,0,fCheckOverlaps);
  ...
  for(int iPlate = 0; iPlate < plates ; iPlate++)
  {
    ...
    if(useTeflonBox)
    {
      ...
      name.str("");
      name << "extBox_" << iPlate;
      G4Box* extBox = new G4Box(name.str().c_str(),fTeflonExt_x/2.0,fTeflonExt_y/2.0,fTeflonExt_z/2.0);
      name.str("");
      name << "intBox_" << iPlate;
      G4Box* intBox = new G4Box(name.str().c_str(),fTeflonInt_x/2.0,fTeflonInt_y/2.0,fTeflonInt_z/2.0);
      G4ThreeVector zTransBox(0, 0, -(fTeflonExt_z-fTeflonInt_z)/2.0 );

      // fTeflonExt_z = ((pMPPCArray_z + fMPPCArray_z/2.0) - (pFakeAirBack_z + fFakeAirBack_z/2.0)) + esrTeflonGap + teflonThickness;
      // fTeflonInt_z = ((pMPPCArray_z + fMPPCArray_z/2.0) - (pFakeAirBack_z + fFakeAirBack_z/2.0)) + esrTeflonGap;
      // G4ThreeVector zTransBox(0, 0, 0);

      name.str("");
      name << "TeflonBox_" << iPlate;
      G4SubtractionSolid* teflonBox = new G4SubtractionSolid(name.str().c_str(), extBox, intBox,0, zTransBox);
      name.str("");
      name << "TeflonLog_" << iPlate;
      G4LogicalVolume* teflonLog = new G4LogicalVolume(teflonBox,                       // shape element
                                                       teflon,      //FIXME                    // material
                                                       name.str().c_str(),                     // name
                                                       0,                               // field manager
                                                       0,                               // sensitive detector
                                                       0);                              // user limits
      G4VisAttributes* teflonVis = new G4VisAttributes(G4Colour(1.0,1.0,1.0));          // white
      teflonLog->SetVisAttributes(teflonVis);                                           // set vis color

      // fTeflonExt_z/2.0 =
      G4double teflonShiftX = 0.;
      G4double teflonShiftY = 0.;
      G4double teflonShiftZ =  -( (fTeflonInt_z/2.0 - (fTeflonExt_z-fTeflonInt_z)/2.0 ) - (pMPPCArray_z + fMPPCArray_z/2.0) );
      // G4double teflonShiftZ = 0.0;

      G4PVPlacement* teflonPhy = new G4PVPlacement(0,                                    // rotation wrt mother volume
                                                   G4ThreeVector(teflonShiftX,
                                                                 teflonShiftY,
                                                                 teflonShiftZ),          // translation wrt mother
                                                   teflonLog,                            // associated logical volume
                                                   "TeflonPhy",                          // name
                                                   bubble_log[iPlate],                          // associated mother log volume
                                                   false,                                // not used,set to false
                                                   0,                                    // copy number
                                                   fCheckOverlaps);                      // if true, check overlaps
      // ******* CREATE A WRAP OPTICAL SURFACE *********
      G4MaterialPropertiesTable *teflonSurfPropertiesTable = new G4MaterialPropertiesTable();
      const G4int Nteflon = 2;
      G4double teflonEnergy[Nteflon] = {2.038*eV, 4.144*eV};
      G4double teflonlobeVector[Nteflon] = {0., 0.};
      G4double teflonSpecularspikeVector[Nteflon] = {0., 0.};
      G4double teflonBackscatterVector[Nteflon] = {0.,0.};
      G4double teflonReflectivityVector[Nteflon] = {teflonReflectivity,teflonReflectivity};
      teflonSurfPropertiesTable->AddProperty("SPECULARLOBECONSTANT",teflonEnergy,teflonlobeVector,Nteflon);
      teflonSurfPropertiesTable->AddProperty("SPECULARSPIKECONSTANT",teflonEnergy,teflonSpecularspikeVector,Nteflon);
      teflonSurfPropertiesTable->AddProperty("BACKSCATTERCONSTANT",teflonEnergy,teflonBackscatterVector,Nteflon);
      teflonSurfPropertiesTable->AddProperty("REFLECTIVITY", teflonEnergy, teflonReflectivityVector, Nteflon);
      // if(latsurfaceroughness != 0)
      //   crystalDepolished_surf->AddConstProperty("SURFACEROUGHNESS", latsurfaceroughness);

      name.str("");
      name << "TeflonSurface_" << iPlate;
      G4OpticalSurface* teflonSurface = new G4OpticalSurface(name.str().c_str());
      teflonSurface->SetType(dielectric_metal);
      teflonSurface->SetFinish(ground);
      teflonSurface->SetModel(unified);
      teflonSurface->SetSigmaAlpha(0.1);
      teflonSurface->SetMaterialPropertiesTable(teflonSurfPropertiesTable);
      name.str("");
      name << "TeflonSurfaceLogicalB_" << iPlate;
      G4LogicalBorderSurface* temp_logical_border = new G4LogicalBorderSurface(name.str().c_str(),
                                                                               expHall_phys,
                                                                               teflonPhy,
                                                                               teflonSurface);
      name.str("");
    ...
    if(fGreaseFrontCryToGlass_z != 0)
    {
      ...
      G4VPhysicalVolume* greaseFrontCryToGlass_phys = new G4PVPlacement(0,G4ThreeVector(pGreaseFrontCryToGlass_x,pGreaseFrontCryToGlass_y,pGreaseFrontCryToGlass_z),greaseFrontCryToGlass_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    if(fGlassFront_z != 0)
    {
      ...
      G4VPhysicalVolume* glassFront_phys = new G4PVPlacement(0,G4ThreeVector(pGlassFront_x,pGlassFront_y,pGlassFront_z),glassFront_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    if(fGreaseFrontGlassToMPPC_z != 0)
    {
      ...
      G4VPhysicalVolume* greaseFrontGlassToMPPC_phys = new G4PVPlacement(0,G4ThreeVector(pGreaseFrontGlassToMPPC_x,pGreaseFrontGlassToMPPC_y,pGreaseFrontGlassToMPPC_z),greaseFrontGlassToMPPC_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    if(fGreaseFrontCryToGlass_z == 0 && fGlassFront_z == 0 && fGreaseFrontGlassToMPPC_z == 0)
    {
      ...
      G4Box* greaseFrontGlassToMPPC_box = new G4Box(name.str().c_str(),fGreaseFrontGlassToMPPC_x/2.0,fGreaseFrontGlassToMPPC_y/2.0,0.01/2.0);
    ...
    if(fEpoxy_z != 0)
    {
      ...
      G4VPhysicalVolume* epoxy_phys = new G4PVPlacement(0,G4ThreeVector(pEpoxy_x,pEpoxy_y,pEpoxy_z),epoxy_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    G4VPhysicalVolume* MPPCArray_phys = new G4PVPlacement(0,G4ThreeVector(pMPPCArray_x,pMPPCArray_y,pMPPCArray_z),MPPCArray_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    //output position
    G4cout << "MPPC array " << iPlate <<" position" << G4endl;
    ...
    for(G4int i = 0 ; i < nDetectorsX ; i++)
    {
      for(G4int j = 0 ; j < nDetectorsY ; j++)
      {
        ...
        detector_phys[i][j] = new G4PVPlacement(0,G4ThreeVector(i*fSingleMPPCBlock_x - mppcShiftX ,j*fSingleMPPCBlock_y - mppcShiftY,mppcShiftZ),detector_log[i][j],name.str().c_str(),MPPCArray_log,false,0,fCheckOverlaps);
        detectorNum++;
      }
    }
    ...
    if(fGreaseBackCryToGlass_z != 0)
    {
      ...
      G4VPhysicalVolume* greaseBackCryToGlass_phys = new G4PVPlacement(0,G4ThreeVector(pGreaseBackCryToGlass_x,pGreaseBackCryToGlass_y,pGreaseBackCryToGlass_z),greaseBackCryToGlass_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    if(fGlassBack_z != 0)
    {
      ...
      G4VPhysicalVolume* glassBack_phys = new G4PVPlacement(0,G4ThreeVector(pGlassBack_x,pGlassBack_y,pGlassBack_z),glassBack_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    G4VPhysicalVolume* airBack_phys = new G4PVPlacement(0,G4ThreeVector(pAirBack_x,pAirBack_y,pAirBack_z),airBack_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    G4VPhysicalVolume* fakeAirBack_phys = new G4PVPlacement(0,G4ThreeVector(pFakeAirBack_x,pFakeAirBack_y,pFakeAirBack_z),fakeAirBack_log,name.str().c_str(),bubble_log[iPlate],false,0,fCheckOverlaps);
    ...
    for(int iCry = 0; iCry < nCrystalsX*nCrystalsY ; iCry++) //create the crystals
    {
      //create a temporary elemet, set its features, then push back to std::vector<Element_t> element
      Element_t temp_element;
      //set number
      temp_element.number = iCry;
      // set position in mother volume (expHall)
      // calculate position.
      // we always follow the x-y plane coordinates, which means element 0 is the one with lower x and y
      // then 1 is same x but higher y, and so on. therefore:
      double elX = (iCry/nCrystalsY)*fAirThinLayerBox_x - matrixShiftX;
      double elY = (iCry % nCrystalsY)*fAirThinLayerBox_y - matrixShiftY;
      double elZ = matrixShiftZ;
      temp_element.x = elX;
      temp_element.y = elY;
      temp_element.z = elZ;

      name.str("");
      name << "ElementBox_" << iPlate << "_" << iCry;
      //box volume
      temp_element.box = new G4Box(name.str().c_str(),                             // box name
                                   fAirThinLayerBox_x/2.0,                         // half dimension on x
                                   fAirThinLayerBox_y/2.0,                         // half dimension on y
                                   fAirThinLayerBox_z/2.0);                        // half dimension on z
      // logical volume
      name.str("");
      name << "ElementLog_" << iPlate << "_" << iCry;
      temp_element.logicalVolume = new G4LogicalVolume(temp_element.box,           // shape element
                                                       airThinLayer,               // material
                                                       name.str().c_str(),         // name
                                                       0,                          // field manager
                                                       0,                          // sensitive detector
                                                       0);                         // user limits
      G4VisAttributes* ElementVis = new G4VisAttributes(G4Colour(1.0,0.0,1.0));    // magenta
      temp_element.logicalVolume->SetVisAttributes(ElementVis);                    // set vis color

      //physical volume

      name.str("");
      name << "ElementPhy_" << iPlate << "_" << iCry;
      temp_element.physicalVolume = new G4PVPlacement(0,                           // rotation wrt mother volume
                                                      G4ThreeVector(temp_element.x,
                                                                    temp_element.y,
                                                                    temp_element.z), // translation wrt mother
                                                      temp_element.logicalVolume,  // associated logical volume
                                                      name.str().c_str(),          // name
                                                      bubble_log[iPlate],                 // associated mother log volume
                                                      false,                       // not used,set to false
                                                      0,                           // copy number
      ...
      // inside each element, a crystal

      name.str("");
      name << "CrystalBox_" << iPlate << "_" << iCry;
      temp_element.crystal.x = 0;
      temp_element.crystal.y = 0;
      temp_element.crystal.z = 0;
      temp_element.crystal.number = 0;
      //box volume
      temp_element.crystal.box = new G4Box(name.str().c_str(),                             // box name
                                           fCrystal_x/2.0,                         // half dimension on x
                                           fCrystal_y/2.0,                         // half dimension on y
                                           fCrystal_z/2.0);                        // half dimension on z
      // logical volume
      name.str("");
      name << "CrystalLog_" << (iCry/nCrystalsY) << "_" << (iCry%nCrystalsY);
      temp_element.crystal.logicalVolume = new G4LogicalVolume(temp_element.crystal.box,           // shape element
                                                       LYSO,                       // material
                                                       name.str().c_str(),         // name
                                                       0,                          // field manager
                                                       0,                          // sensitive detector
                                                       0);                         // user limits
      G4VisAttributes* CryVis = new G4VisAttributes(G4Colour(0.0,1.0,1.0));    // cyan
      temp_element.crystal.logicalVolume->SetVisAttributes(CryVis);                    // set vis color
      ...
      temp_element.crystal.physicalVolume = new G4PVPlacement(0,                           // rotation wrt mother volume
                                                      G4ThreeVector(temp_element.crystal.x,
                                                                    temp_element.crystal.y,
                                                                    temp_element.crystal.z),                    // translation wrt mother
                                                      temp_element.crystal.logicalVolume,  // associated logical volume
                                                      name.str().c_str(),          // name
                                                      temp_element.logicalVolume,                 // associated mother log volume
                                                      false,                       // not used,set to false
                                                      0,                           // copy number
                                                      fCheckOverlaps);             // if true, check overlaps
      ...
      //
      crystalNumber++;
      // create the 4 lateral volumes
      //
      // i.e. the volumes attached to the lateral faces of this crystal
      // also the esr volumes
      ...
      // calc dimension and shift of lateral volumes
      ...
      else if(esrgapx > 0 && esrgapy == 0) // x direction with esrgap, y without
      {
        ...
        if((iCry%nCrystalsY)==0) //first row of crystals, the x dim will need to have no gap if k = 2
          dim_x_esr[2] = fAirThinLayerBox_x - 2.0*(airThinThickness/2.0 - airgap);
        if((iCry%nCrystalsY)==(nCrystalsY-1))//last row of crystals, the x dim will need to have no gap if k = 0
          dim_x_esr[0] = fAirThinLayerBox_x - 2.0*(airThinThickness/2.0 - airgap);
      ...
      else if(esrgapx == 0 && esrgapy > 0)// y direction with esrgap, x without
      {
        ...
        if((iCry/nCrystalsY)==0) //first column of crystals, the y dim will need to have no gap if k = 3
          dim_y_esr[3] = fAirThinLayerBox_y - 2.0*(airThinThickness/2.0 - airgap);
        if((iCry/nCrystalsY)==(nCrystalsX-1))//last column of crystals, the y dim will need to have no gap if k = 1
      ...
      else if(esrgapx > 0 && esrgapy > 0)
      {
        ...
        if((iCry/nCrystalsY)==0) //first row of crystals, the x dim will need to have no gap if k = 3
        {
          dim_y_esr[3] = fAirThinLayerBox_y;
        ...
        if((iCry%nCrystalsY)== 0 && (iCry/nCrystalsY)!= 0 && (iCry/nCrystalsY)!= (nCrystalsX-1))             dim_x_esr[2] = fAirThinLayerBox_x;
        if((iCry%nCrystalsY)== (nCrystalsY-1) && (iCry/nCrystalsY)!= 0 && (iCry/nCrystalsY)!= (nCrystalsX-1))   dim_x_esr[0] = fAirThinLayerBox_x;
      }

      // now create the 4 lateral volumes

      for(int iLat = 0; iLat < 4 ; iLat++)
      {
        Lateral_t temp_lateral;
        name.str("");
        name << "LateralBox_" << iPlate << "_" << iCry << "_" << iLat;
        //box volume
        temp_lateral.box = new G4Box(name.str().c_str(),                             // box name
                                             dim_x[iLat],                         // half dimension on x
                                             dim_y[iLat],                         // half dimension on y
                                             fAirThinLayerBox_z/2.0);                        // half dimension on z
        // logical volume
        name.str("");
        name << "LateralLog_" << iPlate << "_" << iCry << "_" << iLat;
        temp_lateral.logicalVolume = new G4LogicalVolume(temp_lateral.box,           // shape element
                                                         airThinLayer,                       // material
                                                         name.str().c_str(),         // name
                                                         0,                          // field manager
                                                         0,                          // sensitive detector
                                                         0);                         // user limits
        G4VisAttributes* LatVis = new G4VisAttributes(G4Colour(1.0,0.0,0.0));    // red
        temp_lateral.logicalVolume->SetVisAttributes(LatVis);                    // set vis color

        //physical volume

        name.str("");
        name << "LateralPhy_" << iPlate << "_" << iCry << "_" << iLat;
        temp_lateral.physicalVolume = new G4PVPlacement(0,                           // rotation wrt mother volume
                                                        G4ThreeVector(shift_x[iLat],
                                                                      shift_y[iLat],
                                                                      0.),                    // translation wrt mother
                                                        temp_lateral.logicalVolume,  // associated logical volume
                                                        name.str().c_str(),          // name
                                                        temp_element.logicalVolume,  // associated mother log volume
                                                        false,                       // not used,set to false
                                                        0,                           // copy number
        ...
        temp_element.lateral.push_back(temp_lateral);


        EsrBox_t temp_esr;
        name.str("");
        name << "EsrBox_" << iPlate << "_" << iCry << "_" << iLat;
        //box volume
        temp_esr.box = new G4Box(name.str().c_str(),                             // box name
                                             dim_x_esr[iLat]/2.0,                         // half dimension on x
                                             dim_y_esr[iLat]/2.0,                         // half dimension on y
                                             fAirThinLayerBox_z/2.0);                        // half dimension on z
        // logical volume
        name.str("");
        name << "EsrlLog_" << iPlate << "_" << iCry << "_" << iLat;
        temp_esr.logicalVolume = new G4LogicalVolume(temp_esr.box,           // shape element
                                                         airThinLayer,                       // material
                                                         name.str().c_str(),         // name
                                                         0,                          // field manager
                                                         0,                          // sensitive detector
                                                         0);                         // user limits
        G4VisAttributes* EsrVis = new G4VisAttributes(G4Colour(0.0,0.0,1.0));    // blue
        temp_esr.logicalVolume->SetVisAttributes(EsrVis);                    // set vis color

        //physical volume

        name.str("");
        name << "EsrPhy_" << iPlate << "_" << iCry << "_" << iLat;
        temp_esr.physicalVolume = new G4PVPlacement(0,                           // rotation wrt mother volume
                                                        G4ThreeVector(shift_x_esr[iLat],
                                                                      shift_y_esr[iLat],
                                                                      0.),                    // translation wrt mother
                                                        temp_esr.logicalVolume,  // associated logical volume
                                                        name.str().c_str(),          // name
                                                        temp_element.logicalVolume,  // associated mother log volume
                                                        false,                       // not used,set to false
                                                        0,                           // copy number
        ...
        temp_element.esr.push_back(temp_esr);

        // optical surfaces
        if(latdepo_sideBySide[iLat]) // if the surface between crystal and this lateral element is depolished. otherwise polished and only the indexes of refraction will play a role
        {

          std::stringstream Surfname;
          Surfname << "Cry_to_Lat_" << iPlate << "_" << iCry << "_" << iLat;
          G4OpticalSurface* temp_surf = new G4OpticalSurface(Surfname.str().c_str());
          temp_surf->SetType(dielectric_dielectric);
          temp_surf->SetFinish(ground);
          temp_surf->SetModel(unified);
          temp_surf->SetSigmaAlpha(latsigmaalpha);
          temp_surf->SetMaterialPropertiesTable(crystalDepolished_surf);
          G4LogicalBorderSurface* temp_logical_border = new G4LogicalBorderSurface(Surfname.str().c_str(),
                                                                                   temp_element.crystal.physicalVolume,
                                                                                   temp_element.lateral[iLat].physicalVolume,
                                                                                   temp_surf);
          temp_element.cry_to_lat_surface.push_back(temp_surf);
          temp_element.cry_to_lat_logical_surface.push_back(temp_logical_border);


          //and the reverse...
          Surfname << "_Inv" << iPlate << "_";
          ...
          temp_surf_inv->SetMaterialPropertiesTable(crystalDepolished_surf);
          G4LogicalBorderSurface* temp_logical_border_inv = new G4LogicalBorderSurface(Surfname.str().c_str(),
                                                                                       temp_element.lateral[iLat].physicalVolume,
                                                                                       temp_element.crystal.physicalVolume,
                                                                                       temp_surf_inv);
          temp_element.lat_to_cry_surface.push_back(temp_surf_inv);
          temp_element.lat_to_cry_logical_surface.push_back(temp_logical_border_inv);
        }

        //now only for the esr boxes that are on the edge to the world, add a surface to the world
        //we need a surface towards the world, but only for the edge esr boxes.
        //same as before to find them
        std::vector<int> indexEsr;
        if((iCry/nCrystalsY)==0) //crystals on the first column, so the volumes iLat=3 need to have a surface to the world
          indexEsr.push_back(3);
        if((iCry/nCrystalsY)==(nCrystalsX-1) ) //crystals on the last column, so the volumes iLat=1 need to have a surface to the world
          indexEsr.push_back(1);
        if((iCry%nCrystalsY)==0)//crystals on the first row, so the volumes iLat=2 need to have a surface to the world
          indexEsr.push_back(2);
        if((iCry%nCrystalsY)== (nCrystalsY-1) )//crystals on the last row, so the volumes iLat=0 need to have a surface to the world
          indexEsr.push_back(0);


        if(lateralEsr)
        {
          std::stringstream Surfname;
          Surfname << "Air_to_Esr_" << iPlate << "_" << iCry << "_" << iLat;
          G4OpticalSurface* temp_surf = new G4OpticalSurface(Surfname.str().c_str());
        ...
        if(lateralEsr)
        {
          ...
          {
            std::stringstream Surfname;
            Surfname << "Esr_to_World" << iPlate << "_" << iCry << "_" << iLat << "_" << indexEsr[t];
            G4OpticalSurface* temp_surf = new G4OpticalSurface(Surfname.str().c_str());
            temp_surf->SetType(dielectric_metal);
            temp_surf->SetFinish(polished);
            temp_surf->SetModel(unified);
        ...
        if(lateralEsr)
        {
          //front
          name.str("");
          name << "EsrFronSurface_" << iPlate << "_" << iCry << "_" << iLat ;
          temp_element.esr_surf_front  = new G4OpticalSurface(name.str().c_str());
          temp_element.esr_surf_front = new G4OpticalSurface(name.str().c_str());
          temp_element.esr_surf_front->SetType(dielectric_metal);
          temp_element.esr_surf_front->SetFinish(polished);
          temp_element.esr_surf_front->SetModel(unified);
          // temp_element.esr_surf_front->SetSigmaAlpha(realsigmaalpha);
          temp_element.esr_surf_front->SetMaterialPropertiesTable(ESR_surf); //this sets the surface roughness
          temp_element.esr_logical_surf_front = new G4LogicalBorderSurface(name.str().c_str(),
                                                                                 temp_element.esr[iLat].physicalVolume,
                                                                                 volumeInFront,
                                                                                 temp_element.esr_surf_front);
          //and inverse...
          name << "_Inv" << iPlate << "_";
          temp_element.esr_surf_front_inv = new G4OpticalSurface(name.str().c_str());
          temp_element.esr_surf_front_inv->SetType(dielectric_metal);
          temp_element.esr_surf_front_inv->SetFinish(polished);
          temp_element.esr_surf_front_inv->SetModel(unified);
          // temp_element.esr_surf_front_inv->SetSigmaAlpha(realsigmaalpha);
          temp_element.esr_surf_front_inv->SetMaterialPropertiesTable(ESR_surf); //this sets the surface roughness
          temp_element.esr_logical_surf_front_inv = new G4LogicalBorderSurface(name.str().c_str(),
                                                                                     volumeInFront,
                                                                                     temp_element.esr[iLat].physicalVolume,
                                                                                     temp_element.esr_surf_front_inv);

          //back
          name.str("");
          name << "EsrBackSurface_" << iPlate << "_" << iCry << "_" << iLat ;
          temp_element.esr_surf_back = new G4OpticalSurface(name.str().c_str());
          temp_element.esr_surf_back->SetType(dielectric_metal);
          temp_element.esr_surf_back->SetFinish(polished);
          temp_element.esr_surf_back->SetModel(unified);
          // temp_element.esr_surf_back->SetSigmaAlpha(realsigmaalpha);
          temp_element.esr_surf_back->SetMaterialPropertiesTable(ESR_surf); //this sets the surface roughness
          temp_element.esr_logical_surf_back = new G4LogicalBorderSurface(name.str().c_str(),
                                                                                temp_element.esr[iLat].physicalVolume,
                                                                                volumeOnTheBack,
                                                                                temp_element.esr_surf_back);
          //and inverse..
          name << "_Inv" << iPlate << "_";
          temp_element.esr_surf_back_inv = new G4OpticalSurface(name.str().c_str());
          temp_element.esr_surf_back_inv->SetType(dielectric_metal);
          temp_element.esr_surf_back_inv->SetFinish(polished);
          temp_element.esr_surf_back_inv->SetModel(unified);
          // temp_element.esr_surf_back_inv->SetSigmaAlpha(realsigmaalpha);
          temp_element.esr_surf_back_inv->SetMaterialPropertiesTable(ESR_surf); //this sets the surface roughness
          temp_element.esr_logical_surf_back_inv= new G4LogicalBorderSurface(name.str().c_str(),
                                                                                    volumeOnTheBack,
                                                                                    temp_element.esr[iLat].physicalVolume,
                                                                                    temp_element.esr_surf_back_inv);
        }

      }


      if(realdepolished) // twoxtwo more surfaces, from the crystal to the volumeInFront and back, and from crystal to volumeOnTheBack and back
      {

        //front
        name.str("");
        name << "CrystalFrontRealSurface_" << iPlate << "_" << iCry ;
        temp_element.real_depo_surf_front  = new G4OpticalSurface(name.str().c_str());
        temp_element.real_depo_surf_front = new G4OpticalSurface(name.str().c_str());
        temp_element.real_depo_surf_front->SetType(dielectric_dielectric);
        temp_element.real_depo_surf_front->SetFinish(ground);
        temp_element.real_depo_surf_front->SetModel(unified);
        temp_element.real_depo_surf_front->SetSigmaAlpha(realsigmaalpha);
        temp_element.real_depo_surf_front->SetMaterialPropertiesTable(crystalReal_surf); //this sets the surface roughness
        temp_element.real_depo_logical_surf_front = new G4LogicalBorderSurface(name.str().c_str(),
                                                                               temp_element.physicalVolume,
                                                                               volumeInFront,
                                                                               temp_element.real_depo_surf_front);
        //and inverse...
        name << "_Inv" << iPlate << "_";
        temp_element.real_depo_surf_front_inv = new G4OpticalSurface(name.str().c_str());
        temp_element.real_depo_surf_front_inv->SetType(dielectric_dielectric);
        temp_element.real_depo_surf_front_inv->SetFinish(ground);
        temp_element.real_depo_surf_front_inv->SetModel(unified);
        temp_element.real_depo_surf_front_inv->SetSigmaAlpha(realsigmaalpha);
        temp_element.real_depo_surf_front_inv->SetMaterialPropertiesTable(crystalReal_surf); //this sets the surface roughness
        temp_element.real_depo_logical_surf_front_inv = new G4LogicalBorderSurface(name.str().c_str(),
                                                                                   volumeInFront,
                                                                                   temp_element.physicalVolume,
                                                                                   temp_element.real_depo_surf_front_inv);

        //back
        name.str("");
        name << "CrystalBackRealSurface_" << iPlate << "_"<< iCry ;
        temp_element.real_depo_surf_back = new G4OpticalSurface(name.str().c_str());
        temp_element.real_depo_surf_back->SetType(dielectric_dielectric);
        temp_element.real_depo_surf_back->SetFinish(ground);
        temp_element.real_depo_surf_back->SetModel(unified);
        temp_element.real_depo_surf_back->SetSigmaAlpha(realsigmaalpha);
        temp_element.real_depo_surf_back->SetMaterialPropertiesTable(crystalReal_surf); //this sets the surface roughness
        temp_element.real_depo_logical_surf_back = new G4LogicalBorderSurface(name.str().c_str(),
                                                                              temp_element.physicalVolume,
                                                                              volumeOnTheBack,
                                                                              temp_element.real_depo_surf_back);
        //and inverse..
        name << "_Inv" << iPlate << "_";
        temp_element.real_depo_surf_back_inv = new G4OpticalSurface(name.str().c_str());
        temp_element.real_depo_surf_back_inv->SetType(dielectric_dielectric);
        temp_element.real_depo_surf_back_inv->SetFinish(ground);
        temp_element.real_depo_surf_back_inv->SetModel(unified);
        temp_element.real_depo_surf_back_inv->SetSigmaAlpha(realsigmaalpha);
        temp_element.real_depo_surf_back_inv->SetMaterialPropertiesTable(crystalReal_surf); //this sets the surface roughness
        temp_element.real_depo_logical_surf_back_inv= new G4LogicalBorderSurface(name.str().c_str(),
                                                                                  volumeOnTheBack,
                                                                                  temp_element.physicalVolume,
                                                                                  temp_element.real_depo_surf_back_inv);
      }
